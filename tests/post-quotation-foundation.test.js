...
test('quotation has one PDF-backed registry for all 37 dynamic highlighted fields and safely escapes every value',()=>{
  const renderer=read('quotation-renderer.js'),registrySource=read('quotation-field-definitions.js'),latex=read('quotation-latex.js'),styles=read('style.css'),html=read('index.html'),master=read('quotation/LUMA_Quotation.tex'),layout=read('quotation/quotation-field-layout.tex'),defaults=read('quotation/quotation_variables.tex');
  ...
  assert.match(renderer,/quotation\/assets\/LUMA_ENG_static\.pdf/);
  ...
  assert.match(master,/QuotationPageOneOverlay/);
  assert.match(master,/QuotationPageElevenOverlay/);
  assert.match(layout,/anchor=#1/);
  assert.match(latex,/function fittedFontSize/);
  assert.match(latex,/overflowWarnings/);
  assert.equal((defaults.match(/\\newcommand/g)||[]).length,37);
  assert.doesNotMatch(html,/docx-preview|pizzip|jszip/i);
  assert.match(html,/quotation-model\.js/);
  assert.match(html,/quotation-field-definitions\.js/);
  assert.match(html,/quotation-latex\.js/);
  assert.match(html,/quotation\/compilers\/quotation-compiler\.js/);
  assert.match(html,/quotation\/compilers\/swiftlatex-compiler\.js/);
  assert.match(html,/quotation-renderer\.js/);
